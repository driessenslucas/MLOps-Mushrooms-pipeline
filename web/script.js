...
// Function to upload mushroom data
async function uploadMushroomData(name, imageFile) {
	const formData = new FormData();
	formData.append('name', name);
	formData.append('image', imageFile);

	const response = await fetch('http://localhost:8000/upload/mushroom', {
		method: 'POST',
		body: formData,
	});
...
// Function to load mushroom history
async function loadMushroomHistory() {
	const response = await fetch('http://localhost:8000/mushroom/history');
	const history = await response.json();

	// Update history tab in your frontend
...
history.forEach((item) => {
	...
	listItem.addEventListener('click', () => {
		// Code to handle when a history item is clicked
		// For example, you could load the mushroom image and details into the main view
		loadMushroomDetails(item);
	});
	historyList.appendChild(listItem);
});

// Function to load mushroom details when a history item is clicked
function loadMushroomDetails(item) {
	// Update the main view with the selected mushroom's details
	// This is just a placeholder, replace with actual implementation
	document.getElementById('mushroomImage').src = item.image;
	document.getElementById('mushroomName').textContent = item.name;
	document.getElementById('mushroomType').textContent = 'Mushroom Family';
	document.getElementById('mushroomDescription').textContent = item.description;
}
